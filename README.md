...
| ----------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------ |
| **PostgreSQL**    | Основная реляционная база данных. Хранит пользователей, фильмы, жанры, актёров и др. | Структурированные данные удобно хранить и обрабатывать через SQL.        |
| **ClickHouse**    | Аналитическая БД. Хранит просмотры, лайки, рейтинги и агрегированные метрики.        | Быстрое выполнение аналитических запросов на больших объемах данных.     |
| **Kafka**         | Очередь сообщений, обработка потоковых данных. Используется для сбора активности.    | Обеспечивает надёжную и масштабируемую обработку действий пользователей. |
| **Redis**         | Хранилище ключ-значение. Используется для хранения сессий.                           | Очень быстрое хранилище для токенов и авторизации.                       |
| **Tarantool**     | In-memory хранилище. Используется как кэш между PostgreSQL и фронтом.                | Ускоряет ответы за счёт уменьшения нагрузки на основную БД.              |
| **S3 (MinIO)**    | Объектное хранилище. Сюда загружаются фильмы, трейлеры, аватары и постеры.           | Удобно и дёшево хранить большие бинарные файлы.                          |
| **NginX**         | Обратный прокси, балансировщик нагрузки, сервер статики.                             | Раздаёт фронт, балансирует нагрузку и проксирует запросы к backend'у.    |
| **ElasticSearch** | Поисковый движок. Используется для полнотекстового поиска по фильмам и актёрам.      | Обеспечивает быстрый и удобный поиск по названиям и описаниям.           |
| **Odyssey**       | Прокси для PostgreSQL. Балансирует и управляет подключениями.                        | Снижает нагрузку и оптимизирует работу с PostgreSQL.                     |
| **k8s**           | Оркестрация контейнеров. Управление backend-сервисами и масштабирование.             | Обеспечивает стабильность, автообновления и масштабируемость.            |
