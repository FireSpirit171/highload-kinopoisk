...
- Для каждого пользователя находится K-ближайших соседей по косинусному сходству их векторов интересов (`genre_vector` и `person_vector`).
...
| **ElasticSearch** | Поиск                     | Обеспечивает быстрый и удобный поиск по названиям и описаниям.                                  |
| **Odyssey**       | Прокси для PostgreSQL     | Снижает нагрузку и оптимизирует работу с PostgreSQL.                                            |
| **k8s**           | Оркестрация контейнеров   | Обеспечивает стабильность, автообновления и масштабируемость.                                   |
| **Prometheus**    | Мониторинг                | Сбор метрик для анализа состояния системы и ресурсов.                                           |
| **Grafana**       | Визуализация метрик       | Наглядное отображение данных мониторинга для быстрого реагирования.                             |
...
|                            | **Что используется**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Базы данных**            | Для PostgreSQL, ClickHouse и Elasticsearch реализованы шардирование и репликация, а также настраиваются регулярные бэкапы для защиты от потери данных.                                                                                                                                                                                                                                                                                                                                                           |
| **Виртуальные компоненты** | - **Kubernetes** следит за состоянием подов, перезапуская их при сбоях. <br>- **Nginx** развернут с несколькими репликами, что позволяет продолжить работу даже при отказе одного из инстансов. <br>- **etcd** сохраняет текущее состояние и конфигурации кластера, что ускоряет восстановление в случае сбоев.                                                                                                                                                                                                  |
| **Физические компоненты**  | - **Серверы** распределяются по ролям и работают в пределах Kubernetes-кластера, что обеспечивает изоляцию и устойчивость к сбоям. <br>- **Диски** работают на RAID 5, обеспечивая восстановление данных при выходе из строя одного накопителя.                                                                                                                                                                                                                                                                  |
| **Мониторинг метрик**      | Метрики передаются через Kafka и собираются системой Prometheus, а **Grafana** используется для визуализации и настройки алертов. Это позволяет оперативно выявлять неполадки и контролировать ключевые показатели производительности.                                                                                                                                                                                                                                                                           |
| **Graceful Shutdown**      | Все сервисы корректно обрабатывают SIGTERM/SIGINT сигналы Kubernetes: при остановке сервиса завершаются текущие запросы, закрываются подключения к базам данных, и только после этого контейнер завершает работу. Это предотвращает потерю данных и ошибок при обновлении или падении подов.                                                                                                                                                                                                                     |
| **Graceful Degradation**   | <table><thead><tr><th>Недоступный сервис</th><th>Что показываем вместо</th></tr></thead><tbody><tr><td>Сервис рекомендаций</td><td>Подборку новых фильмов</td></tr><tr><td>Сервис оценок и комментариев</td><td>Сообщение "Комментарии недоступны"</td></tr><tr><td>Сервис поиска</td><td>Популярные жанры/категории</td></tr><tr><td>Сервис изображений</td><td>Заглушка "Изображение недоступно"</td></tr><tr><td>Сервис просмотра видео</td><td>Сообщение "Видео сейчас недоступно"</td></tr></tbody></table> |
| **Паттерны**               | Используется паттерн **Circuit Breaker**: если один из сервисов (например, рекомендаций) временно недоступен, API Gateway перестаёт направлять к нему запросы, чтобы избежать перегрузки системы и каскадных отказов. Реализация [Circuit Breaker на Go](https://github.com/sony/gobreaker).                                                                                                                                                                                                                     |
...
- **Сервис карточки контента** — ходит в PostgreSQL за информацией о филмье/сериале, его актерах и фильмах.
...
| ------------------------------------------------------------- | ----------------------------------- | --- | ------- | ---------------- |
| Сервис авторизации/регистрации (Тяжелая бизнес-логика)        | 30 RPS                              | 30  | 6000 MB | 0.036 Gbit/s     |
| Сервис оплаты подписки (Тяжелая бизнес-логика)                | 0.3 RPS                             | 1   | 200 MB  | 0.00036 Gbit/s   |
| Сервис поиска контента (Средняя бизнес-логика)                | 120 RPS                             | 2   | 120 MB  | 0.432 Gbit/s     |
| Сервис просмотра видео (Средняя бизнес-логика)                | 2708 RPS                            | 28  | 2708 MB | 194.176 Gbit/s   |
| Сервис отображения изображений (Легкое JSON API)              | 120 RPS                             | 1   | 50 MB   | 3.84 Gbit/s      |
| Сервис рекомендаций (Средняя бизнес-логика)                   | 30 RPS                              | 1   | 30 MB   | 0.108 Gbit/s     |
| Сервис оценок и комментариев (Средняя бизнес-логика)          | 8.1 + 0.0005 RPS                    | 1   | 100 MB  | 0.029 Gbit/s     |
| Сервис пользователей (Легкое JSON API)                        | 12 RPS                              | 1   | 50 MB   | 0.0384 Gbit/s    |
| Сервис добавления контента (Легкое JSON API)                  | <0.0001 RPS                         | 1   | 1 MB    | ~0.00001 Gbit/s  |
| Сервис сбора метрик (Легкое JSON API)                         | 110 RPS                             | 1   | 20 MB   | 0.528 Gbit/s     |
| Сервис обновления рекомендаций (Kafka, фоновая обработка)     | 30 RPS                              | 1   | 100 MB  | 0.1 Gbit/s       |
| Сервис модерации (Kafka, фоновая обработка)                   | 0.0005 RPS                          | 1   | 100 MB  | ~0.000002 Gbit/s |
| Сервис обновления рейтинга фильмов (Kafka, фоновая обработка) | 8.1 RPS                             | 1   | 100 MB  | 0.0324 Gbit/s    |
...
- Тяжелая бизнес-логика: 1 CPU на 1 RPS.
- Средняя бизнес-логика: 1 CPU на 100 RPS.
- Лёгкое JSON API: 1 CPU на 500 RPS.
...
Для оркестрации будет использоваться **Kubernetes (k8s)**.

| Сервис                                     | Количество контейнеров     |
| ------------------------------------------ | -------------------------- |
| Количество API Gateway                     | 5 ДЦ \* 3 API Gateway = 15 |
| Количество NGINX                           | 30                         |
| Сервис авторизации/регистрации             | 4                          |
| Сервис оплаты подписки                     | 2                          |
| Сервис поиска контента                     | 4                          |
| Сервис просмотра видео                     | 14                         |
| Сервис отображения изображений             | 2                          |
| Сервис рекомендаций                        | 2                          |
| Сервис оценок и комментариев               | 2                          |
| Сервис пользователей                       | 2                          |
| Сервис добавления контента                 | 1                          |
| Сервис сбора метрик                        | 2                          |
| Сервис обновления рекомендаций (Kafka)     | 1                          |
| Сервис модерации (Kafka)                   | 1                          |
| Сервис обновления рейтинга фильмов (Kafka) | 1                          |

### Пояснение:

Принятые нормы загрузки одного контейнера:

- Для тяжёлой логики: контейнер тянет ~8–10 RPS.
- Для средней логики: контейнер тянет ~100–150 RPS.
- Для лёгкой логики: контейнер тянет ~500 RPS.

## Выбор модели хостинга

Выбрана модель **собственного железа**, поскольку проект имеет крупный масштаб и хорошо прогнозируемую нагрузку.

## Конфигурации

| Сервис                                     | CPU/req | CPU/lim | RAM/req | RAM/lim | Кол-во |
| ------------------------------------------ | ------- | ------- | ------- | ------- | ------ |
| Сервис авторизации/регистрации             | 8       | 11      | 1600 MB | 3300 MB | 4      |
| Сервис оплаты подписки                     | 1       | 2       | 200 MB  | 400 MB  | 2      |
| Сервис поиска контента                     | 1       | 2       | 120 MB  | 300 MB  | 4      |
| Сервис просмотра видео                     | 3       | 5       | 300 MB  | 600 MB  | 14     |
| Сервис отображения изображений             | 1       | 2       | 50 MB   | 100 MB  | 2      |
| Сервис рекомендаций                        | 1       | 2       | 30 MB   | 60 MB   | 2      |
| Сервис оценок и комментариев               | 1       | 2       | 100 MB  | 200 MB  | 2      |
| Сервис пользователей                       | 1       | 2       | 50 MB   | 100 MB  | 2      |
| Сервис добавления контента                 | 1       | 1       | 1 MB    | 2 MB    | 1      |
| Сервис сбора метрик                        | 1       | 2       | 20 MB   | 40 MB   | 2      |
| Сервис обновления рекомендаций (Kafka)     | 1       | 2       | 100 MB  | 200 MB  | 1      |
| Сервис модерации (Kafka)                   | 1       | 2       | 100 MB  | 200 MB  | 1      |
| Сервис обновления рейтинга фильмов (Kafka) | 1       | 2       | 100 MB  | 200 MB  | 1      |

### Пример расчёта параметров для сервиса авторизации/регистрации

1. **CPU request (CPU/req)**:

   - Сервис тяжёлой бизнес-логики.
   - Норма: 1 CPU на 1 RPS.
   - Нагрузка: 30 RPS.
