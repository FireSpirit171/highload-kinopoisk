...
Учитывая большую территорию России разместим 10 дополнительных дата центров для работы на ее территории в крупных городах. После географического анализа были выбраны следующие:
...
Дополнительно, для Казахстана добавлен дата-центр в **Алматы**, чтобы улучшить покрытие в этом регионе, а также возможного (в будущем) выхода на рынок стран Средней Азии.
...
**Связи между ДЦ для проксирования (можно отметить стрелками):**
- **Ростов-на-Дону → Минск**
- **Сургут → Санкт-Петербург**
- **Петропавловск-Камчатский → Екатеринбург**
...
Использование **Latency-based DNS (GeoDNS)** позволит автоматически направлять запросы пользователей к ближайшему дата-центру с минимальным временем отклика (RTT). Размещение ДЦ выбрано с учетом этой схемы.

## Регулировка трафика между дата-центрами

В случае перегрузки ближайшего ДЦ трафик перераспределяется по принципу **latency-based balancing**. Например, если пользователь из **Улан-Удэ** отправляет запрос в **Иркутск**, но тот перегружен, запрос будет перенаправлен на ДЦ с минимальной задержкой, например, в **Омск**. В критических случаях этот процесс может продолжаться в несколько итераций, обеспечивая оптимальную маршрутизацию трафика.
